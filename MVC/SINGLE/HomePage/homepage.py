...
DATABASE = '/home/fanhaipeng/fanipeng-homepage-cache.sqlite'
DEBUG = True
...
def data_is_stale():
    ...
    try:
        last_updated = g.db.cursor().execute(
            'select last_refresh from entries order by last_refresh desc limit 1').fetchone()[0]
    except:
        return True
...
def load_twitter():
    twitter = feedparser.parse(
        "http://twitter.com/statuses/user_timeline/14377703.rss")
    g.db.cursor().execute('DELETE FROM entries WHERE source = "twitter"')

    for entry in twitter.entries:
        g.db.cursor(
        ).execute('INSERT INTO entries VALUES (?, ?, ?, ?, ?, ?, ?)',
                  (None,
                   entry['link'],
...
def load_picasa():
    picasa = feedparser.parse(
        "http://picasaweb.google.com/data/feed/base/user/thisdyingdream/albumid/5501252408388252785?alt=rss&kind=photo&hl=en_US")
...
def load_delicious():
    delicious = feedparser.parse(
        "http://feeds.delicious.com/v2/rss/stevenharms?count=10")
